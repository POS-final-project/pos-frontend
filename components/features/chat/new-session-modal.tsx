...
export function NewSessionModal({
  ...
  const [scope, setScope] = useState<"global" | "shop">(
    allowGlobal ? "global" : "shop",
  );
  const [shopId, setShopId] = useState(forcedShopId ?? "");
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-4 pt-2">
          ...
          {/* Scope cards — hanya tampil jika global diperbolehkan */}
          {allowGlobal ? (
            ...
            <div className="flex items-center gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
              ...
              <p className="text-sm text-amber-800 font-medium">
                Percakapan terbatas pada toko Anda
              </p>
            </div>
          )}

          {/* Shop picker */}
          {scope === "shop" && !forcedShopId && (
